...
def load_yaml(settings_path, knowned_dataset, knowned_backbone):
    args = {}
    with open(settings_path) as data_file:
        param = yaml.load(data_file, Loader=yaml.FullLoader)
    args.update(param['basic'])

    dataset = args['dataset'] if knowned_dataset is None else knowned_backbone
    backbone = args['backbone'] if knowned_backbone is None else knowned_backbone
    if 'options' in param:
...
class Config:
    ...
    def __init__(self, config_mode=None):
        ...
        parser.add_argument('--openset_test', action="store_true", default=None, help='apply openset test or not.')
        parser.add_argument('--test_per_epoch', action="store_true", default=None, help='test per epoch.')
        ...
        if args_part.config is not None:
            init_config = load_yaml(self.config, self.dataset, self.backbone) # obtain configs from yaml
        ...
        elif self.method_type is not None and self.method is not None:
            # get specific configs from (the rest of) command args
            modellib = importlib.import_module('methods.' + self.method_type + '.' + self.method)
            if hasattr(modellib, 'add_special_args'):
                modellib.add_special_args(parser)
            for name, value in vars(parser.parse_args()).items():
                setattr(self, name, value)
        elif self.checkpoint_dir is not None:
            splited_checkpoint_dir = self.checkpoint_dir.split('/')
            if 'multi_steps' in splited_checkpoint_dir or 'multi_step' in splited_checkpoint_dir:
                self.method_type = 'multi_steps'
            elif 'single_step' in splited_checkpoint_dir:
                self.method_type = 'single_step'
            for i in range(len(splited_checkpoint_dir)):
                if splited_checkpoint_dir[i] in self.method_type:
                    self.method = re.sub("_cil_replay.*$", "", splited_checkpoint_dir[i+1])
                    self.method = re.sub("_til_replay.*$", "", self.method)
                    break
        ...
        if self.checkpoint_dir:
            ...
            if self.method_type == 'multi_steps':
                logdir = 'logs/{}/{}_{}_replay_{}/{}/{}_b{}i{}'.format(self.method_type, self.method, self.incre_type, 
